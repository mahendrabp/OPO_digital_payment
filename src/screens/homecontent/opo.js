...
class OPO extends Component {
  ...
  getUser = async () => {
    ...
    this.setState({
      user: this.props.user.getUser[0],
  ...
  handleSubmit = async () => {
    if (this.state.phone !== '' && this.state.nominal !== '') {
      await this.props.dispatch(
        transfer(this.state.phone, this.state.nominal, this.state.user.user_id),
      );
      // console.log(this.state.saldo)
      // console.log(this.props.user.transfer)
      this.setState({
        saldo: parseInt(this.state.saldo) - parseInt(this.state.nominal),
      });
    }
    this.setState({
      phone: '',
